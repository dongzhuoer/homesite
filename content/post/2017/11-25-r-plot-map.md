...
以前某次英语课展示（应该是外教的）时，我想在地图上标一些经纬度。然后就想到与其手动加在图片上，肯定是用 R 代码来得更精确（并无卵用，纯属装逼）。后来逐渐积累了一些画地图的方法，但至今没怎么用上，就现在放在这里。由于数据文件（`data-raw/res1_4m.shp`）较大，有些还要用到网络，我就不保证可重复了。
