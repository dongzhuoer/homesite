...
我迟迟没掌握 Python 的一个原因就是没有好用的 IDE（主要原因是我的 R 已经~~炉火纯青~~勉强够用了）。Wing 沉迷于找破解码（Python2 和 Python3 我都有），PyCharm 则执着于比较 Professional ^[南开邮箱可以申请教育许可证] 和 Community 的区别以及方便的安装方式 ^[一度觉得 Snap 不错，除了网速慢]，直到 VSCode 横空出世、统一了所有语言 ^[我用它开发过 R，C++ 和 Python]。
